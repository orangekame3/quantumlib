...
def plot_s_vs_phase(phase_range, S_values, save_path=None, show_theory=True):
    ...
    ax1.axhline(
        y=-tsirelson_bound, color="green", linestyle=":", alpha=0.8, linewidth=2
    )
    ...
    if show_theory and len(phase_zoom) > 1:
        ...
        ax2.plot(
            phi_zoom_theory, S_zoom_theory, "r-", linewidth=3, alpha=0.8, label="Theory"
        )
...
def analyze_phase_dependence(phase_range, S_values, verbose=True):
    ...
    if verbose:
        ...
        print("\nExtreme Values:")
        print(
            f"  Maximum: S = {max_S:.3f} at φ = {max_phase:.3f} rad ({np.degrees(max_phase):.1f}°)"
        )
        print(
            f"  Minimum: S = {min_S:.3f} at φ = {min_phase:.3f} rad ({np.degrees(min_phase):.1f}°)"
        )
        print(f"  Range: ΔS = {max_S - min_S:.3f}")

        print("\nBell Inequality Violation:")
        print(
            f"  Phases with |S| > 2: {violation_count}/{len(S_values)} ({violation_percentage:.1f}%)"
        )
        ...
        print("\nPhysical Insights:")
        print("🌊 Phase dependence shows wave-like quantum correlations")
        print("📊 Maximum violation at φ = 0 (S = +2√2), φ = π (S = -2√2)")
        print("⚖️ Zero violation at φ = π/2, 3π/2 (S = 0)")
        print("🔄 Sinusoidal behavior: S = 2√2 cos(φ)")
...
if __name__ == "__main__":
    ...
    print(
        f"🎯 Violation percentage: {results['analysis']['violation_percentage']:.1f}%"
    )
