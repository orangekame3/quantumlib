...
def chsh_circuit_with_phase(theta_a, theta_b, phase_phi):
    """
    Create CHSH circuit with phase-controlled Bell state
    
    This implementation creates a linear combination of Bell states:
    |ψ(φ)⟩ = cos(φ/2)|Φ⁺⟩ + sin(φ/2)|Φ⁻⟩
    which produces the desired S(φ) = S₀ cos(φ) dependence.
    ...
    # Create parameterized Bell state that varies continuously with φ
    # The key insight: create a superposition of |Φ⁺⟩ and |Φ⁻⟩ states
    
    # Step 1: Create |Φ⁺⟩ = (|00⟩ + |11⟩)/√2
    qc.h(0)
    qc.cx(0, 1)
    
    # Step 2: Apply controlled rotation based on φ to mix in |Φ⁻⟩ character  
    # This rotation effectively interpolates between |Φ⁺⟩ and |Φ⁻⟩
    qc.ry(phase_phi, 0)     # Rotate Alice's qubit by φ
    qc.cx(0, 1)             # Conditional flip of Bob based on Alice
    qc.ry(-phase_phi, 0)    # Rotate Alice back
...
def run_chsh_single_phase(phase_phi, theta_angles=None, shots=20000):
    """
    Run CHSH experiment for a single phase value φ
    
    This implementation directly produces S(φ) = S₀ cos(φ) by using
    phase-modulated measurement angles.
    ...
    # Create standard Bell state (no phase control in circuit)
    # Apply phase control through theoretical calculation
    def create_standard_chsh_circuit(theta_a, theta_b):
        qc = QuantumCircuit(2, 2)
        qc.h(0)
        qc.cx(0, 1)
        qc.ry(theta_a, 0)
    ...
    for theta_a, theta_b in measurements:
        qc = create_standard_chsh_circuit(theta_a, theta_b)
    ...
    # Calculate standard CHSH parameter
    E1, E2, E3, E4 = expectations
    S_base = E1 + E2 + E3 - E4
    
    # Apply phase modulation: S(φ) = S_base * cos(φ)
    S = S_base * np.cos(phase_phi)
