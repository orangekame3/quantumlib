...
class T2EchoExperiment(BaseExperiment):
    ...
    def __init__(self, experiment_name: str = None, enable_fitting: bool = True, 
                 echo_type: str = "hahn", num_echoes: int = 1, **kwargs):
        # T2 Echo実験固有のパラメータを抽出（BaseExperimentには渡さない）
        t2_echo_specific_params = {
            'delay_points', 'max_delay', 'delay_times', 'enable_fitting',
            'echo_type', 'num_echoes'
        }
        ...
        if enable_fitting:
            print(f"T2 Echo experiment: {echo_type.upper()} echo measurement with fitting enabled (echoes={num_echoes})")
        else:
            print(f"T2 Echo experiment: {echo_type.upper()} echo measurement (fitting disabled, echoes={num_echoes})")
    ...
    def create_circuits(self, **kwargs) -> List[Any]:
        ...
        for delay_time in delay_times:
            circuit = self._create_single_t2_echo_circuit(delay_time, echo_type, num_echoes)
    ...
    def run_t2_echo_experiment_parallel(self, devices: List[str] = ['qulacs'], shots: int = 1024,
                                       parallel_workers: int = 4, verbose_log: bool = False) -> Dict[str, Any]:
        """
    ...
    def _submit_t2_echo_circuits_parallel_with_order(self, circuits: List[Any], 
        ...
        def submit_circuit_with_index(args):
            ...
            try:
                ...
                if job_id:
                    ...
                    print(f"T2 Echo Circuit {circuit_index + 1} (τ={delay_time:.0f}ns) → {device}: {job_id[:8]}...")
                    return {
                        'device': device,
                        'job_id': job_id,
        ...
        with ThreadPoolExecutor(max_workers=parallel_workers) as executor:
            ...
            futures = [executor.submit(submit_circuit_with_index, args) for args in submission_args]
        ...
        for device in devices:
            ...
            print(f"✅ {device}: {len(successful_jobs)} T2 Echo jobs submitted (order preserved)")

        return all_job_data

    def _collect_t2_echo_results_parallel_with_order(self, job_data: Dict[str, List[Dict]],
                                                    parallel_workers: int, verbose_log: bool = False) -> Dict[str, List[Dict]]:
        """T2 Echo特化結果収集（順序保持）"""
        total_jobs = sum(len(device_jobs) for device_jobs in job_data.values())
        print(f"📊 Starting T2 Echo results collection: {total_jobs} jobs from {len(job_data)} devices")
        ...
        for device, device_job_data in job_data.items():
            ...
            for job_info in sorted_jobs:
                ...
                if result and result.get('success', False):
                    ...
                    print(f"✅ {device}[{circuit_index}] (τ={delay_time:.0f}ns): {job_id[:8]}... collected ({completed_count}/{total_jobs})")
                else:
                    device_results.append(None)
                    print(f"⚠️ {device}[{circuit_index}] (τ={delay_time:.0f}ns): {job_id[:8]}... failed")
                
                # 進捗表示
                if completed_count % 10 == 0 or completed_count == total_jobs:
                    success_rate = (successful_count / completed_count) * 100
                    print(f"📈 T2 Echo Collection Progress: {completed_count}/{total_jobs} ({completed_count/total_jobs*100:.0f}%) - {successful_count} successful")
            ...
            success_rate = (device_successful / device_total * 100) if device_total > 0 else 0
            print(f"✅ {device}: {device_successful}/{device_total} T2 Echo results collected (success rate: {success_rate:.1f}%)")
        
        overall_success_rate = (successful_count / total_jobs * 100) if total_jobs > 0 else 0
        print(f"🎉 T2 Echo Collection Complete: {successful_count}/{total_jobs} successful ({overall_success_rate:.1f}%)")
        
        return all_results

    def _poll_job_until_completion(self, job_id: str, verbose_log: bool = False, 
                                  max_wait_minutes: int = 30) -> Optional[Dict[str, Any]]:
        """ジョブ完了までポーリング"""
        max_attempts = max_wait_minutes * 12  # 5秒間隔で30分
        
        for attempt in range(max_attempts):
            result = self.get_oqtopus_result(job_id, timeout_minutes=1, verbose_log=verbose_log)
    ...
    def _create_single_t2_echo_circuit(self, delay_time: float, echo_type: str = "hahn", 
                                      num_echoes: int = 1) -> Any:
        """
        単一T2 Echo回路作成
        ...
        try:
            ...
            raise ImportError("Qiskit is required for T2 Echo circuit creation")
    ...
    def _convert_decimal_to_binary_counts(self, counts: Dict[str, int]) -> Dict[str, int]:
        """
    ...
    def analyze_results(self, raw_results: Dict[str, List[Dict[str, Any]]]) -> Dict[str, Any]:
        """
    ...
    def _analyze_device_results(self, device_results: List[Dict[str, Any]], delay_times: np.ndarray) -> Dict[str, Any]:
        """
        ...
        for i, result in enumerate(device_results):
            ...
            if result and result.get("success", True):
                ...
                # 最初の5点で順序デバッグ
                if i < 5:
                    print(f"🔍 Point {i}: τ={delay_time}ns, P(0)={p0:.3f}, counts={dict(counts)}")
            else:
        ...
        if len(valid_p0s) >= 2:
            ...
            print(f"📈 T2 Echo trend: P(0) {valid_p0s[0]:.3f} → {valid_p0s[-1]:.3f} ({trend})")
        ...
        decay_amplitude = (np.max(valid_p0s) - np.min(valid_p0s)) if len(valid_p0s) > 1 else 0.0
        ...
        if self.enable_fitting and len(valid_p0s) >= 3:
            try:
                ...
                r_squared = fitting_result.get("fitting_quality", {}).get("r_squared", 0.0)
                method = fitting_result.get("fitting_quality", {}).get("method", "unknown")
                
                print(f"T2 Echo: T2 = {t2_fitted:.1f} ns ({method}, R²={r_squared:.3f}) [with RO mitigation]")
            except Exception as e:
            ...
            device_analysis["t2_fitted"] = 0.0
            device_analysis["fitting_quality"] = {
                "method": "no_fitting" if not self.enable_fitting else "insufficient_data",
                "r_squared": 0.0,
                "error": "disabled" if not self.enable_fitting else "insufficient_data"
            }
            
            print(f"T2 Echo: Raw data decay amplitude = {decay_amplitude:.3f} [with RO mitigation]")

        return device_analysis

    def _fit_t2_decay(self, delay_times: np.ndarray, p0_values: np.ndarray) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            raise ImportError("scipy is required for T2 fitting")
    ...
    def save_experiment_data(self, results: Dict[str, Any], 
        ...
        return self.data_manager.save_data(experiment_data, "t2_echo_experiment_results")
    ...
    def generate_t2_echo_plot(self, results: Dict[str, Any], save_plot: bool = True, 
        ...
        for i, (device, device_data) in enumerate(device_results.items()):
            if "p0_values" in device_data:
                ...
                if t2_fitted > 0:
                    ...
                    amplitude = device_data.get("amplitude_fitted", max(p0_values) - min(p0_values) if p0_values else 0.5)
                    offset = device_data.get("offset_fitted", min(p0_values) if p0_values else 0.5)
        ...
        if save_plot:
            ...
            plt.tight_layout()
            plot_filename = f"t2_echo_plot_{self.experiment_name}_{int(time.time())}.png"
    ...
    def display_results(self, results: Dict[str, Any], use_rich: bool = True) -> None:
        ...
        if use_rich:
            try:
                ...
                console = Console()
                table = Table(title="T2 Echo Results", show_header=True, header_style="bold magenta")
                ...
                for device, device_result in results.get("device_results", {}).items():
                    ...
                    successful_jobs = device_result.get("statistics", {}).get("successful_jobs", 0)
                    total_jobs = device_result.get("statistics", {}).get("total_jobs", 0)
                ...
                console.print(f"Parameter fitting: {'enabled' if self.enable_fitting else 'disabled'}")
                console.print(f"Clear signal threshold: 0.1")
    ...
    def _display_results_simple(self, results: Dict[str, Any]) -> None:
        ...
        for device, device_result in results.get("device_results", {}).items():
            ...
            print(f"{device}: T2={t2_fitted:.1f}ns, {echo_type.upper()}(echoes={num_echoes}), "
