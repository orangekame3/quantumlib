...
Advanced CHSH experiment CLI for research (typer-based)
"""
...
# Import after path setup (required for module discovery)
from src.quantumlib import CHSHExperiment  # noqa: E402
        ...
        for shots in track(shot_counts, description="Testing shot counts..."):
            ...
            if "analyzed_results" in result:
                for device, analysis in result["analyzed_results"][
                    ...
                    console.print(
                        f"   📊 {device}: |S|={max_s:.3f}, violations={violations}"
                    )
    ...
    def display_theoretical_comparison(self, results: dict, phases: list[float]):
        ...
        for device, device_analysis in analysis["device_results"].items():
            ...
            for _i, (phase, theo_s, exp_s) in enumerate(
                zip(phases, theoretical_s, experimental_s, strict=False)
            ):
                if not np.isnan(exp_s):
                    ...
                    style = (
                        "green"
                        if rel_error < 10
                        else "yellow" if rel_error < 25 else "red"
                    )
    ...
    def generate_research_report(self):
        ...
        for i, entry in enumerate(self.experiment_log, 1):
            ...
            if "devices" in summary:
                exp_node.add(f"📱 Devices: {', '.join(summary['devices'])}")
...
def study(
    ...
    shots: Annotated[int, typer.Option(help="🎲 Number of measurement shots")] = 2000,
    topic: Annotated[
        str, typer.Option(help="📝 Research topic name")
    ] = "chsh_research",
    verbose: Annotated[
    ...
    try:
        if mode == ResearchMode.bell_study:
            research.run_bell_study(device_list, shots, points=50)
            ...
            research.run_noise_robustness(device_list, shots)

        elif mode == ResearchMode.theoretical_comparison:
            research.run_theoretical_comparison(device_list, shots)
        ...
        console.print(
            f"📁 Results saved in: {research.base_exp.data_manager.session_dir}"
        )
...
def batch(
    config_file: Annotated[
        Path, typer.Argument(help="📄 Batch configuration file (JSON)")
    ],
    verbose: Annotated[
        bool, typer.Option("--verbose", "-v", help="🔍 Verbose output")
    ] = False,
    ...
    try:
        ...
        for experiment_config in config.get("experiments", []):
            ...
            devices = [
                DeviceType(d) for d in experiment_config.get("devices", ["qulacs"])
            ]
